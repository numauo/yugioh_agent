...
# -*- coding: utf-8 -*-
"""
Yu-Gi-Oh! Duel Agent (CLI) powered by Groq Qwen3-32B

This script wires the Groq-hosted Qwen3-32B chat model into a small command line
interface that can drive a duel against the user. It mirrors the behaviour of
the local Hugging Face variant but delegates generation to Groq's HTTP API.

Usage example:
    python Qwen3-32B_groq.py \
        --prompt-file ../prompt_yugioh.txt \
...
- ターンごとにLP・フィールドの公開情報をまとめ、ユーザーが入力しやすい形で質問する。
...
def strip_json_comments(text: str) -> str:
    """Remove line and block comments from JSONC-like text."""
...
# ====== メイン ======
def main() -> None:
    parser = argparse.ArgumentParser(description="Yu-Gi-Oh! duel agent using Groq Qwen3-32B.")
    ...
    parser.add_argument("--request-timeout", type=float, default=float(os.environ.get("REQUEST_TIMEOUT", "120")),
